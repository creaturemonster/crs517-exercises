...
//
//
//  Super Shopping Cart for Ex 6.1 and later
//
//
...
public class ShoppingCartServlet extends HttpServlet {
	...
	/**
	 *  Retrieves an instance of the VideoDAO and MusicDAO
	 *
	 *  @throws ServletException error occurred connecting to VideoDAO
	 */
	public void init() throws ServletException {

		// Retrieve the VideoDAO and MusicDAO from the servlet context
		// The VideoDAO and MusicDAO was initialized by the listener class: RainAppListener
		...
		try {
			...
			if (command.equals("add")) {
				...
				if (productType.equalsIgnoreCase("video")) {
					theProduct = myVideoDAO.getVideoRecording(productId);
				} else if (productType.equalsIgnoreCase("music")) {
					theProduct = myMusicDAO.getMusicRecording(productId);
				}
	...
	public void showCart(ShoppingCart userCart, PrintWriter out, HttpServletResponse response) {
		...
		String musicDetailsLink = "music/music_details.jsp?id=";
		...
		for (ShoppingCartItem tempItem : items) {
			...
			else if (tempProduct instanceof MusicRecording) {
				productIcon = " <img src=images/music-icon.gif alt=Music> ";
				productLink = productIcon + "<a href=" + musicDetailsLink + tempProduct.getId() + ">" + tempProduct.getName() + "</a>";
			}
