...
public class VideoCatalogServlet extends HttpServlet {
	...
	/**
	 *  Reference to the RainforestService accessor
	 */
	...
	protected void buildHtmlTable(List<VideoRecording> theList, PrintWriter out, String category, HttpServletResponse response) {
		...
		String titleLink = "<a href=" + response.encodeURL(baseLink + "&sort_by=" + Dao.SORT_BY_TITLE) + ">Title</a>";
		String priceLink = "<a href=" + response.encodeURL(baseLink + "&sort_by=" + Dao.SORT_BY_PRICE) + ">Price</a>";
		String stockCountLink = "<a href=" + response.encodeURL(baseLink + "&sort_by=" + Dao.SORT_BY_STOCK_COUNT) + ">Stock Count</a>";
